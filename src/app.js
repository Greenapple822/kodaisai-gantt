...
SharedStore.prototype.parse = function (text) {
  ...
  return { csv: text, layout: null, names: null, at: null, by: '' };
};
...
SharedStore.prototype.save = function (csv, layout, by) {
  var self = this;
  if (!this.url) return Promise.reject(new Error('共有先が設定されていません'));
  var body = JSON.stringify({ csv: csv, layout: layout || null, at: Date.now(), by: by || '' });
...
VariantStore.prototype.create = function (csv, want, layout) {
  var v = { name: this.uniqueName(want || ('案' + (this.list.length + 1))), csv: csv,
            layout: layout || null, at: Date.now() };
  this.list.push(v);
...
VariantStore.prototype.put = function (csv, layout) {
  if (this.isShared()) { this.sharedCsv = csv; this.sharedLayout = layout || null; return null; }
  var v = this.active();
  if (!v) {
    if (csv === this.base && !layout) return null;   /* 何も変えていない */
    return this.create(csv, '案1', layout);
  }
  v.csv = csv;
...
function VariantBar(store, onLoad, sharedEnabled) {
  ...
  document.getElementById('variant-new').addEventListener('click', function () {
    store.create(self.getCsv(), '', store.currentLayout());
    self.render();
...
VariantBar.prototype.autoSave = function (csv, layout) {
  if (this.store.isShared()) {          /* 共有の保存は main 側が受け持つ */
    this.store.put(csv, layout);
    return true;
  }
  var wasActive = this.store.activeName;
  var v = this.store.put(csv, layout);
...
function main() {
  ...
  loadData().then(function (d) {
    ...
    function refresh() {
      var csv = rerender();
      variantBar.autoSave(csv, layoutOverride);
    ...
    function flushSharedSave() {
      ...
      pendingCsv = null;
      savingNow = true;
      shared.save(csv, layoutOverride, who()).then(function (at) {
        savingNow = false;
    ...
    /* サーバーの内容を画面に入れる */
    function adoptShared(csv, remoteLayout, legacyNames) {
      store.sharedCsv = csv;
    ...
    function pollShared() {
      ...
      shared.load().then(function (got) {
        ...
        if (mine === (store.sharedCsv || originalCsv)) {
          adoptShared(got.csv, got.layout, got.names);                      /* 自分は何も変えていない。黙って追従。 */
    ...
    document.getElementById('shared-load').addEventListener('click', function () {
      shared.load().then(function (got) { if (got) adoptShared(got.csv, got.layout, got.names); });
    });
    ...
    function refreshShared() {
      ...
      shared.load().then(function (got) {
        ...
        if (got) {
          adoptShared(got.csv, got.layout, got.names);
          variantBar.showShared('saved', { at: got.at, by: got.by });
        } else {
