...
/* 会場エリアの表示名だけを差し替えたレイアウトを作る。
   id と座標は layout.json のまま。schedule.csv の area は id を指しているので壊れない。 */
function effectiveLayout(base, names) {
  names = names || {};
  return {
    viewBox: base.viewBox,
    areas: (base.areas || []).map(function (a) {
      var o = {};
      Object.keys(a).forEach(function (k) { o[k] = a[k]; });
      if (names[a.id]) o.name = names[a.id];
      return o;
    })
  };
}

/* 既定の名前と違うものだけを残す。全部持つと layout.json を差し替えづらい。 */
function diffNames(base, names) {
  var out = {};
  (base.areas || []).forEach(function (a) {
    var v = names[a.id];
    if (v && v !== a.name) out[a.id] = v;
  });
  return out;
}
...
function AreaNamesView(opts) {
  ...
  this.host.addEventListener('input', function (e) {
    ...
    if (v) self.names[id] = v; else delete self.names[id];
    self.onChange();
  });
}

AreaNamesView.prototype.setData = function (base, names) {
  this.base = base;
  this.names = names || {};
  this.render();
};
...
AreaNamesView.prototype.render = function () {
  var self = this;
  this.host.innerHTML = (this.base.areas || []).map(function (a) {
    var id = 'area-name-' + a.id;
    return '<div class="area-row">'
      + '<label for="' + id + '">' + esc(a.name) + '</label>'
      + '<input type="text" id="' + id + '" data-area="' + esc(a.id) + '" '
      + 'value="' + esc(self.names[a.id] || a.name) + '" aria-label="' + esc(a.name) + ' の表示名">'
      + '</div>';
  }).join('');
};
...
SharedStore.prototype.parse = function (text) {
  ...
  try {
    ...
    if (o && typeof o.csv === 'string') {
      return { csv: o.csv, names: o.names || {}, at: o.at || null, by: o.by || '' };
    }
  } catch (e) { /* JSON でなければ素の CSV とみなす */ }
  return { csv: text, names: {}, at: null, by: '' };
};
...
SharedStore.prototype.save = function (csv, names, by) {
  var self = this;
  if (!this.url) return Promise.reject(new Error('共有先が設定されていません'));
  var body = JSON.stringify({ csv: csv, names: names || {}, at: Date.now(), by: by || '' });
...
/* 場所の名前も予定の一部として一緒に持ち運ぶ */
VariantStore.prototype.currentNames = function () {
  if (this.isShared()) return this.sharedNames || {};
  var v = this.active();
  return (v && v.names) || {};
};
...
VariantStore.prototype.create = function (csv, want, names) {
  var v = { name: this.uniqueName(want || ('案' + (this.list.length + 1))), csv: csv,
            names: names || {}, at: Date.now() };
  this.list.push(v);
...
VariantStore.prototype.put = function (csv, names) {
  names = names || {};
  var renamed = Object.keys(names).length > 0;
  if (this.isShared()) { this.sharedCsv = csv; this.sharedNames = names; return null; }
  var v = this.active();
  if (!v) {
    if (csv === this.base && !renamed) return null;   /* 何も変えていない */
    return this.create(csv, '案1', names);
  }
  v.csv = csv;
  v.names = names;
  v.at = Date.now();
...
function VariantBar(store, onLoad, sharedEnabled) {
  ...
  document.getElementById('variant-new').addEventListener('click', function () {
    store.create(self.getCsv(), '', store.currentNames());
    self.render();
...
VariantBar.prototype.autoSave = function (csv, names) {
  if (this.store.isShared()) {          /* 共有の保存は main 側が受け持つ */
    this.store.put(csv, names);
    return true;
  }
  var wasActive = this.store.activeName;
  var v = this.store.put(csv, names);
...
function main() {
  ...
  loadData().then(function (d) {
    var baseLayout = d.layout;          /* layout.json そのもの。id と座標はここが正 */
    var areaNames = {};                 /* 画面で変えた表示名だけを持つ */
    var layout = baseLayout;            /* 表示に使う（名前を差し替えたもの） */
    ...
    areaNames = store.currentNames();
    layout = effectiveLayout(baseLayout, areaNames);
    ...
    var areaView = new AreaNamesView({
      ...
      onChange: function () { applyNames(); }
    });
    areaView.setData(baseLayout, areaNames);

    /* 名前を直したら、表の「場所」欄と図の両方に反映する */
    function applyNames() {
      areaNames = diffNames(baseLayout, areaView.names);
      layout = effectiveLayout(baseLayout, areaNames);
    ...
    function refresh() {
      var csv = rerender();
      variantBar.autoSave(csv, areaNames);
    ...
    function flushSharedSave() {
      ...
      pendingCsv = null;
      savingNow = true;
      shared.save(csv, areaNames, who()).then(function (at) {
        savingNow = false;
    ...
    /* サーバーの内容を画面に入れる */
    function adoptShared(csv, names) {
      store.sharedCsv = csv;
      store.sharedNames = names || {};
      areaNames = store.sharedNames;
      layout = effectiveLayout(baseLayout, areaNames);
      areaView.setData(baseLayout, areaNames);
      editor.layout = layout;
      editor.setData(normalizeRows(parseCsv(csv)), layout);
    ...
    function pollShared() {
      ...
      shared.load().then(function (got) {
        ...
        if (mine === (store.sharedCsv || originalCsv)) {
          adoptShared(got.csv, got.names);                      /* 自分は何も変えていない。黙って追従。 */
    ...
    document.getElementById('shared-load').addEventListener('click', function () {
      shared.load().then(function (got) { if (got) adoptShared(got.csv, got.names); });
    });
    ...
    setupCsvDialog(function () {
      var changed = Object.keys(areaNames).length > 0;
      return {
        csv: toCsv(editor.rows),
        layout: changed ? JSON.stringify(layout, null, 2) + '\n' : null
      };
    });
